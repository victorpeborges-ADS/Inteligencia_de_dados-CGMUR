...
def test_generate_report_persists_record(mock_plan, _precip, _map, tmp_path):
    mock_plan.return_value = {
        "severidade": "Alta",
        "acoes_tecnicas_padrao": [],
        "capacidade_investimento": {"fontes_financiamento_sugeridas": ["FCP"]},
    }
    fake_weasy = ModuleType("weasyprint")
    fake_html = MagicMock()
    fake_weasy.HTML = MagicMock(return_value=fake_html)
