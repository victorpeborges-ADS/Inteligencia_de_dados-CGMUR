...
def start_onboarding_batch_job(
    ...
    job_id = run_onboarding_batch_job(limit=min(limit, 8), status_filter=status, force=force)
...
def start_mapbiomas_batch_job(
    ...
    job_id = run_mapbiomas_batch_job(limit=min(limit, 8), force=force)
    return {"job_id": job_id, "job": get_job(job_id)}


@router.post("/jobs/pipeline")
def start_pipeline_job(
    onboarding_limit: int = 8,
    _admin: User = Depends(require_role(Role.ADMIN)),
):
    from app.services.background_jobs import get_job, run_pipeline_job

    job_id = run_pipeline_job(onboarding_limit=min(onboarding_limit, 8))
    return {"job_id": job_id, "job": get_job(job_id)}
...
def start_diagnostics_batch_job(
    ...
    job_id = run_diagnostics_batch_job(limit=min(limit, 8), codigos=codes)
...
def start_reports_batch_job(
    ...
    codes = [c.strip().zfill(7)[:7] for c in codigos.split(",") if c.strip()] if codigos else None
    job_id = run_reports_batch_job(limit=min(limit, 8), force=force, codigos=codes)
    return {"job_id": job_id, "job": get_job(job_id), "reused": False}
...
def start_external_sources_batch_job(
    ...
    job_id = run_external_sources_batch_job(limit=min(limit, 8))
    return {"job_id": job_id, "job": get_job(job_id)}
...
def start_bairros_batch_job(
    ...
    job_id = run_bairros_batch_job(limit=min(limit, 8), force=force)
    return {"job_id": job_id, "job": get_job(job_id)}
...
def start_dem_batch_job(
    ...
    job_id = run_dem_batch_job(limit=min(limit, 8), force=force)
    return {"job_id": job_id, "job": get_job(job_id)}
...
def start_homologation_full_job(
    ...
    job_id = run_full_homologation_job(
        onboarding_limit=min(onboarding_limit, 8),
        force_dem=force_dem,
    )
    return {"job_id": job_id, "job": get_job(job_id)}
