...
def rasterize_muni(
    ...
    info = {
        ...
        "source": "PE3D XYZ → GeoTIFF",
        "product": meta.get("product", "mdt"),
    }
...
def main() -> int:
    ap = argparse.ArgumentParser(
        description="Converte XYZ PE3D (MDT ou MDS) em GeoTIFF WGS84 por município.",
    )
    ...
    ap.add_argument(
        ...
        choices=("mdt", "mds"),
        default="mdt",
        help="mdt → {ibge}_lidar.tif (terreno); mds → {ibge}_dsm.tif (superfície)",
    )
    ...
    if not files:
        ...
        return 1
    print(f"{len(files)} tiles em {xyz_dir} (product={args.product})")

    suffix = "lidar" if args.product == "mdt" else "dsm"
    out_dir = Path(args.out_dir)
    for code in args.ibge:
        meta = {**MUNIS[code], "product": args.product}
        out = out_dir / f"{code}_{suffix}.tif"
