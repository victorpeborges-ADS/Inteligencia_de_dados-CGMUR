...
rem =====================================================================
rem  Avaliar SINIDU (Windows)
rem  Duplo clique para checar Docker, API, frontend e municipios-piloto.
...
set "SCRIPT=%PROJECT_DIR%\scripts\avaliar_sistema.py"
set "REPORT=%PROJECT_DIR%\scripts\avaliacao_ultimo_relatorio.txt"
set "EXITCODE=1"
...
if defined PY (
  ...
  docker exec ^
    -e AVALIACAO_SKIP_DOCKER=1 ^
    -e FRONTEND_URL=http://host.docker.internal:3000 ^
    -e AVALIACAO_REPORT_PATH=/tmp/avaliacao_ultimo_relatorio.txt ^
    sinidu_backend python /tmp/avaliar_sistema.py http://127.0.0.1:8000
  set "EXITCODE=!ERRORLEVEL!"
